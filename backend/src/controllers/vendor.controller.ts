  ...
  try {
    ...
    if (paymentMethod === 'CASH') {
      ...
      if (!cashRegister)
        return res
          .status(400)
          .json({ status: 'error', message: 'No tienes caja abierta para pagar en efectivo.' });
    } else if (!accountId) {
      return res
        .status(400)
        .json({ status: 'error', message: 'Debes seleccionar una cuenta bancaria.' });
    }
    ...
    await logAction(
      ...
      'Vendor',
      id,
      `Pago de ${parsedAmount} a proveedor ${vendor.name}.`
    );

    return res
      .status(200)
      .json({ status: 'success', message: 'Pago a proveedor registrado correctamente.' });
  } catch (error: any) {
